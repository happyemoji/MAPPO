    ...
    while len(starts) < M:
        ...
        for t in range(TB):
            for i in range(s_len):
                ...
                # pdb.set_trace()
                tmp_epsilon_x = max_step * random.random() #0-0.1
...
def main():
    ...
    for j in range(N_new):
        ...
        for indices in index_sample:
            for k in indices:
                one_starts_agent.append(copy.deepcopy(one_starts_landmark[k]))
        # pdb.set_trace()
    ...
    for j in range(num_updates):
        ...
        starts = starts + random.sample(starts_old,N_old)
        # 再从starts生成obs
        obs, rou_index= envs.new_starts_obs(starts,now_agent_num)# 需要在starts中剔除不合适的点
        ...
        for i in range(len(Rew)):
            if Rew[i] < Rmin or Rew[i] > Rmax:
                starts[rou_index[i]] = '*'
        length_starts = len(starts)
        i = 0 
        while i < length_starts:
            if starts[i]=='*':
                del starts[i]
                i -= 1
                length_starts -= 1
